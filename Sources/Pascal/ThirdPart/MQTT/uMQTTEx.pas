...
//    mtReserved0,	  //  0	Reserved
  ...
  TMQTTPublishEvent = procedure (Sender : TObject; anID : Word; aTopic : UTF8String; aMessage : RawByteString) of object;
  ...
  public
    ...
    procedure SendPublish (anID : Word; aTopic: UTF8String; aMessage : RawByteString; aQOS : TMQTTQOSType; aDup : boolean = false; aRetain : boolean = false);
    procedure SendPing;
    ...
    //VGS : Adding Objet data to handle custom server/client object inside event.
    Property DataServer : TObject read FDataServer Write FDataServer;
    Property DataClient : TObject read FDataClient Write FDataClient;
    Property DataOut : TObject read FDataOut Write FDataOut;
  end;
...
//  aStream.Write (aStr[1], length (aStr));
  //VGS note : Unicode fix : Pass,User,Topic,ClientID is UTF8. Payload raw bytes.
  b := TStringStream.Create(aStr);
  try
    aStream.CopyFrom(b,b.Size);
  finally
    FreeAndNil(b);
  end;
end;
...
//      aStream.Read (Result[1], l);
      //VGS : UTF8 string decoding.
      b := TStringStream.Create(UTF8String(''));
      try
        b.CopyFrom(aStream,l);
        result := b.DataString;
      finally
        FreeAndNil(b);
      end;
                        ...
                        try
                          while FRxStream.Size >= FRxStream.Position + 3  do   // len + qos
                            begin
                              ...
                              Strs.AddObject (string(aStr), TObject (Pointer(x)));
                            end;
...
begin
  ...
  for i := 0 to Topics.Count - 1 do
    begin
      ...
      AddByte (s, byte (Pointer(Topics.Objects[i])) and $03);
    end;
  // payload
  s.Seek (0, soBeginning);
  AddLength (FTxStream, s.Size);
