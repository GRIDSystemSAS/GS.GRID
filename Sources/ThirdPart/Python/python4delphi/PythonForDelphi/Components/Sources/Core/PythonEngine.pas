...
{$ENDIF}
    raise Exception.Create(ClassName + ' ' + s +' ' + GetQuitMessage);
...
begin
  Result := Format( 'Dll %s could not be loaded.', [DllName]);
end;

procedure TDynamicDll.Quit;
begin
  raise Exception.Create(ClassName + ' ' + GetQuitMessage);
///
...
begin
  ...
  try
    ...
    on E: Exception do
    begin
      raise Exception.Create(ClassName + ' ' + GetQuitMessage + ' /'+E.Message);
    end;
  end;
end;
...
begin
  ...
  s := GetDllPath+aDllName;
  if FileExists(s) then
    FDLLHandle := SafeLoadLibrary(s)
  else
    raise EFileNotFoundException.Create(ClassName + '"'+s+'" not found');
end;
